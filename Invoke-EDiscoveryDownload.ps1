...
.EXAMPLE
    ...
    Tests all download methods for a case matching "ArchiveExport". Prompts for device code auth.

.EXAMPLE
    .\Test-EDiscoveryDownload.ps1 -CaseId "abc123-def456-etc"
    Tests downloads for specific case ID.
...
.NOTES
    ...
    Prerequisites:
    - Microsoft.Graph module (for case/export metadata)
    - MSAL.PS module (for download token acquisition)
    - User must be member of the eDiscovery case
    - User must authenticate interactively (Microsoft limitation - app-only auth not supported for downloads)
...
function Connect-ToGraph
{
    ...
    if ($existingContext)
    {
        ...
        $script:TenantId = $existingContext.TenantId
...
function Get-PurviewDownloadToken
{
    ...
    try
    {
        ...
        # Use the native client redirect URI that should be configured in your app registration
        ...
        Write-Log "  Redirect URI: $redirectUri" -Level DEBUG -Method "TOKEN"

        # Try interactive browser flow directly (device code has delegate issues in PowerShell)
        Write-Log "Opening browser for authentication..." -Method "TOKEN"
        Write-Log "Please sign in when the browser opens" -Level WARNING -Method "TOKEN"
...
function Test-Method1-InvokeWebRequest
{
    ...
    foreach ($url in $script:Results.ExportMetadata.DownloadUrls)
    {
        ...
        try
        {
            ...
            # Suppress progress bar and use basic parsing to avoid prompts
            $prevProgressPref = $ProgressPreference
            $ProgressPreference = 'SilentlyContinue'

            $response = Invoke-WebRequest -Uri $url -Headers $headers -OutFile $outputFile -PassThru -UseBasicParsing -ErrorAction Stop
    ...
    try
    {
        ...
        if ($response -and $response.value)
        {
            ...
            if ($script:PurviewToken)
            {
                ...
                {
                    $ProgressPreference = 'SilentlyContinue'
                    Invoke-WebRequest -Uri $response.value -Headers $headers -OutFile $outputFile -UseBasicParsing -ErrorAction Stop
...
function Test-Method3-AzureBlobDirect
{
    ...
    foreach ($sasUrl in $sasUrls)
    {
        ...
        try
        {
            ...
            $ProgressPreference = 'SilentlyContinue'
            Invoke-WebRequest -Uri $sasUrl -OutFile $outputFile -UseBasicParsing -ErrorAction Stop
...
try
{
    ...
    $script:TenantId = $null  # Will be set from Graph context
